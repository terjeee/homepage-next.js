...
export default function NavMobile() {
  ...
  const paths = [
    ["home", "/", <IconHome height={37.5} width={37.5} />],
    ["projects", "/projects", <IconCode height={37.5} width={37.5} />],
    ["contact", "/contact", <IconMail height={37.5} width={37.5} />],
  ];
  ...
  return (
    <>
      <div className="flex max-h-screen flex-col sm:hidden">
        ...
        {menuOpen && (
          <div className="z-40">
            <div className="absolute inset-0 h-screen bg-white dark:bg-darkBg">
              <div className="flex h-full items-center justify-end pr-6">
                <ul className="flex flex-col gap-6">
                  {paths.map(([title, path, iconComponent], index) => {
                    return (
                      <motion.li initial={{ opacity: 0, translateY: -10 }} animate={{ opacity: 1, translateY: 0 }} transition={{ duration: 0.2 }}>
                        <Link href={String(path)} onClick={toggleMenu} className="flex items-center justify-end gap-2">
